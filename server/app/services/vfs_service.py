...
class VFSService:
    ...
    @classmethod
    async def run_expired_uploads_gc(cls, db: AsyncSession) -> dict:
        """
        背景垃圾回收核心：大掃除所有過期的上傳會話與物理孤立暫存目錄 (雙向盤點大掃除)
        """
        import os
        import logging
        import anyio
        ...
        # 1. 計算過期時間閾值
        # 統一使用無時區 UTC 時間進行比對，防範 SQLite 隱式時區轉換問題
        ...
        try:
            ...
            if expired_sessions:
                ...
                for session in expired_sessions:
                    try:
                        # 呼叫現有的 cancel_upload，重用完整的 DB 刪除與磁碟原子物理清理邏輯
                        await cls.cancel_upload(db, upload_id=session.id, owner_id=session.owner_id)
                        db_cleaned_count += 1
            ...
            err_msg = f"過期會話資料庫查詢大掃除失敗: {e}"
            logger.error(f"[GC] {err_msg}")
            errors.append(err_msg)

        # ==========================================================
        # 階段二：由外而內 (Physical-driven) ── 清理磁碟中的孤立暫存碎片
        # ==========================================================
        # 盤點實體暫存目錄 (/data/temp)，找出所有在 DB 中查無對應，且 mtime 已過期的目錄
        try:
            temp_dir = settings.TEMP_DIR
            
            # 使用 anyio 執行緒池安全地檢查實體目錄存在性
            async def check_temp_dir_exists():
                return os.path.exists(temp_dir)
            
            temp_dir_exists = await anyio.to_thread.run_sync(check_temp_dir_exists)

            if temp_dir_exists:
                # 列出實體磁碟下的所有子目錄
                def list_physical_dirs():
                    try:
                        return [d for d in os.listdir(temp_dir) if os.path.isdir(os.path.join(temp_dir, d))]
                    except Exception:
                        return []

                physical_dirs = await anyio.to_thread.run_sync(list_physical_dirs)

                if physical_dirs:
                    # 查詢目前所有在 DB 中活躍的會話 ID
                    active_stmt = select(UploadSession.id)
                    active_result = await db.execute(active_stmt)
                    active_ids = set(active_result.scalars().all())

                    # 比對出孤立目錄 (實體磁碟存在，但 DB 中沒有任何紀錄)
                    for p_dir in physical_dirs:
                        if p_dir not in active_ids:
                            p_path = os.path.join(temp_dir, p_dir)
                            
                            # 獲取目錄的修改時間 (mtime) 確保不是剛好此時正在發起 init 的新傳輸目錄
                            def get_dir_mtime():
                                try:
                            ...
                            mtime_stamp = await anyio.to_thread.run_sync(get_dir_mtime)
                            if mtime_stamp:
                                mtime = datetime.fromtimestamp(mtime_stamp, timezone.utc).replace(tzinfo=None)
                                
                                # 如果最後修改時間大於過期閾值，安全清除！
                                if mtime < expire_threshold:
                                    logger.warning(f"[GC] 偵測到物理孤立暫存目錄 {p_dir} 且已超時，即將物理超度...")
                                    try:
                                        # 呼叫現有的原子清理工具 filesystem.cleanup_temp
                                        await filesystem.cleanup_temp(p_dir)
                                        physical_cleaned_count += 1
                                    except Exception as err:
                                        err_msg = f"物理清理孤立目錄 {p_dir} 失敗: {err}"
                                        logger.error(f"[GC] {err_msg}")
                                        errors.append(err_msg)
        except Exception as e:
            err_msg = f"物理孤立暫存盤點掃除失敗: {e}"
            logger.error(f"[GC] {err_msg}")
            errors.append(err_msg)

        logger.info(f"[GC] 垃圾回收大掃除完成！共清除 DB 過期會話: {db_cleaned_count} 個，物理孤立目錄: {physical_cleaned_count} 個")
